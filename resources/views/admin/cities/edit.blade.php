            ...
            <div class="card mb-5 mt-5 shadow-1">
                <div class="card-header">
                    <h4 class="card-header-title">
                        {{ __('form.city.edit_city') }}
                    </h4>
                </div>
                <div class="card-body collapse show" id="collapse8">
                    <form class="needs-validation" action="{{ route("cities.update", [$item->id]) }}" method="post">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            <label for="country_id">{{ __('form.role.role') }}</label>
                            <select class="form-control select2 select2-hidden-accessible"
                                    data-placeholder="Choose country" tabindex="-1" aria-hidden="true" id="country_id"
                                    name="country_id">
                                @foreach($countries as $country)
                                    <option
                                        value="{{ $country->id }}" @selected($item->country_id === $country->id)>{{ $country->nameTr }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="text-center">
                            <button class="btn btn-sm btn-success col-md-4"
                                    type="submit">{{ __('table.save') }}</button>
                        </div>
                    </form>
                </div>
            </div>
